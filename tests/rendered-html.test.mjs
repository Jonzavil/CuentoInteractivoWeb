...
test("keeps the story content aligned with the delivered media", async () => {
  ...
  assert.equal(storyData.match(/videoSrc:/g)?.length, 21);
  assert.equal(storyData.match(/posterSrc:/g)?.length, 21);
  assert.equal(storyData.match(/copyBlocks:/g)?.length, 21);
  ...
  assert.equal(animationFiles.filter((file) => file.endsWith(".mp4")).length, 21);
  assert.equal(posterFiles.filter((file) => file.endsWith(".jpg")).length, 21);
